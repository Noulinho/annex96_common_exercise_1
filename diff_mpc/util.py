...
def bdiag(d):
    """
    Build a batch of diagonal matrices from a batch of diagonal vectors.

    Input:
        d: (B, sz)  (can be float or bool)
    Output:
        D: (B, sz, sz) same dtype as d
    """
    assert d.ndimension() == 2
    nBatch, sz = d.size()

    # dtype/device from d
    D = torch.zeros(nBatch, sz, sz, device=d.device, dtype=d.dtype)

    # bool mask for diagonal locations (PyTorch-2 safe)
    I = torch.eye(sz, device=d.device, dtype=torch.bool).unsqueeze(0).expand(nBatch, -1, -1)
